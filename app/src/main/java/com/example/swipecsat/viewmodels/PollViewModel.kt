...
class PollViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun sendAnswer(answer: Int) {
        if (currentPoll.value!!.items[currentItemIndex.value!!] !is Question) return
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            val body = okhttp3.RequestBody
                .create("application/json".toMediaType(),
                    "{\"user_id\": \"${uId.value}\", " +
                            "\"question\": \"${(currentPoll.value!!.items[currentItemIndex.value!!] as Question).text}\", " +
                            "\"answer\": \"$answer\", " +
                            "\"feedback\": \"empty\"}")
    ...
    fun loadQuestions(productName: String) {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val decodedItems = JSONArray(response.body?.string())

                for (i in 0 until decodedItems.length()) {
                    val item = decodedItems.getString(i)
                    items.add(Question(this@PollViewModel, item))
                }
                items.add(EndPoll(this@PollViewModel))

                withContext(Dispatchers.Main) {
                    questionsCount.value = items.count { it is Question }
